...
**Mode** を選ぶと、同一LAN内の他PCからHTTP経由で測定をトリガーできるようになります。

| モード | 待ち受け | GUIの測定・設定系操作 |
|---|---|---|
| **Off**（既定） | しない | 常に操作できる |
| **Standby** | 常時 | APIリクエストが装置を操作している間だけロックされる |
| **Locked** | 常時 | サーバー起動中はずっとロックされる（従来の "Start API Server" と同じ） |

Standbyでは、リモートから使いたくなるたびに実機の前まで行ってサーバーを起動する必要が
ありません。ロックされるのはリクエストの実行中だけで、その間はパネルに
**● Remote control active** が点灯します。選んだモードは保存され、次回起動時に自動で復帰します。

接続に使うAPIキーはメニューバーの **API → Manage Clients...** でクライアントごとに発行・失効
できます。通信はTLSで保護されないため、信頼できるLAN内での運用を前提としてください。
