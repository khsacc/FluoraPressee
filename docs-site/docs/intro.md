...
1. 横軸の較正情報を内部的にデータベース化して管理する機能を備えている。特に、回折格子・中心位置が同一の較正情報をグループ化して管理し、デフォルトでは最新の較正情報だけをスピーディーに適用することができる。また較正に使用したスペクトルやピーク位置の標準として用いた標準試料の情報なども保存しており、あとから較正に疑義が生じた際にその妥当性を検証できる。
